...
Automator::Automator(const ModemManagerData::Settings &settings, QObject *parent)
    : QObject(parent),
      _settings(settings),
      _timer(new QTimer()),
      _managerModemRemoved(
          {AutomatorScript::emptyState, AutomatorScript::Item(State(State::OfonoManagerModemRemoved, State::Signal))},
          this),
...

{
  ...
  connect(_timer.data(), &QTimer::timeout, [this]() { onStatusChanged(AutomatorScript::Finished); });
...
void Automator::processing(QObject *sender, const State &state)
{
  ...
  if (AutomatorScript::emptyState != state)
  {
...
void Automator::onStatusChanged(const AutomatorScript::ScriptStatus status)
{
  ...
  if (script == &_managerModemRemoved)
  {
    D("--- AutomatorScript::StatusChanged ---: ModemRemoved");
    _timer->stop();
    _data.clearAll();
  }
  else if (script == &_managerModemAdded)
  {
  ...
  else if (script == &_connectionContextAdded)
  {
    ...
    if (!(_data.connectionContextAccessPointName.isValid() && _data.connectionContextUsername.isValid() &&
          _data.connectionContextPassword.isValid()))
    {
  ...
  switch (status)
  {
    case AutomatorScript::NotStarted: st = "NotStarted"; break;
    case AutomatorScript::Started: st = "Started"; break;
    case AutomatorScript::Finished: st = "Finished"; break;
    case AutomatorScript::Error: st = "Error"; break;
  }
  */
}

bool Automator::isAutomatorScriptStarted() const
{
  return (AutomatorScript::Started == _managerModemRemoved.status() ||
          AutomatorScript::Started == _managerModemAdded.status() ||
          AutomatorScript::Started == _modemLockdownDisable.status() ||
          AutomatorScript::Started == _modemLockdownEnable.status() ||
          AutomatorScript::Started == _modemPoweredDisable.status() ||
          AutomatorScript::Started == _modemPoweredEnable.status() ||
          AutomatorScript::Started == _modemOnlineDisable.status() ||
          AutomatorScript::Started == _modemOnlineEnable.status() ||
          AutomatorScript::Started == _simManagerAdded.status() ||
          AutomatorScript::Started == _networkRegistrationAdded.status() ||
          AutomatorScript::Started == _connectionManagerAdded.status() ||
          AutomatorScript::Started == _connectionContextAdded.status() ||
          AutomatorScript::Started == _connectionContextAccessPointName.status() ||
          AutomatorScript::Started == _connectionContextUsername.status() ||
          AutomatorScript::Started == _connectionContextPassword.status() ||
          AutomatorScript::Started == _connectionContextActiveDisable.status() ||
          AutomatorScript::Started == _connectionContextActiveEnable.status());
}
