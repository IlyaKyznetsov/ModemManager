...
void ModemManager::_automatorProcessing(const State &state)
{
  switch (state.type())
  {
    ...
    case State::OfonoConnectionContextAccessPointName:
    {
      if (State::CallStarted == _automator.connectionContextAccessPointNameStatus && State::Signal == state.status())
        return;
...
bool ModemManager::isTimeoutError(const QDBusError &error)
{
